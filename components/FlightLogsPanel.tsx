...
${trackpoints}
    ...
    return (
        <div className="mt-8 flex-grow flex gap-6 animate-fade-in h-[calc(100%-100px)]">
            <div className="w-1/3 bg-white p-4 rounded-2xl shadow-sm flex flex-col">
                <h2 className="text-xl font-bold mb-4 px-2">Mission History</h2>
                <div className="px-2 mb-4">
                    <div className="relative">
                        <input 
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gcs-orange"
                        />
                    ...
                    <div className="mt-2">
                        <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 text-sm text-gray-600 hover:text-gcs-orange">
                           <FilterIcon /> 
                     ...
                     {showFilters && (
                        <div className="mt-3 p-4 bg-gray-50 rounded-lg border animate-fade-in-sm">
                            <div className="flex justify-between items-center mb-3">
                                <h4 className="font-bold text-sm text-gray-800">Filter Options</h4>
                                {filtersAreActive && (
                                    <button 
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(['All', 'Completed', 'Interrupted'] as const).map(status => (
                                         <button key={status} onClick={() => setStatusFilter(status)} className={`px-3 py-1 text-xs rounded-full transition-colors ${statusFilter === status ? 'bg-gcs-orange text-white font-bold' : 'bg-gray-200 hover:bg-gray-300 text-gray-700'}`}>
                                            {status}
                                        </button>
                                    ))}
                                </div>
                             ...
                             <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">Breeding Sites</label>
                                 <select onChange={(e) => setBreedingSiteFilter(e.target.value as any)} value={breedingSiteFilter} className="w-full p-2 border bg-white border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-gcs-orange">
                ...
                <div className="overflow-y-auto space-y-2 pr-2 flex-grow">
                    {filteredMissions.map(mission => (
                        <button key={mission.id} 
                            ...
                            className={`w-full text-left p-4 rounded-lg transition-colors ${selectedMission?.id === mission.id ? 'bg-gcs-orange/10 text-gcs-orange' : 'hover:bg-gray-100'}`}
                        >
                            <p className="font-bold">{mission.name}</p>
                            <p className="text-sm text-gray-500">{mission.date} - {mission.duration}</p>
                            <p className={`text-xs font-semibold mt-1 ${mission.status === 'Completed' ? 'text-green-600' : 'text-orange-600'}`}>{mission.status}</p>
                        </button>
                    ))}
            ...
            {/* Mission Details */}
            <div className="flex-1 bg-white p-6 rounded-2xl shadow-sm flex flex-col">
                 {selectedMission ? (
                    <>
                        <h2 className="text-2xl font-bold mb-1">{selectedMission.name} Details</h2>
                        <p className="text-sm text-gray-500 mb-4">{selectedMission.date}</p>
                        ...
                        {selectedMission.detectedSites && selectedMission.detectedSites.length > 0 && (
                            <div className="mt-4 pt-4 border-t">
                                <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wider mb-2">Detected Breeding Sites</h3>
                                <div className="space-y-2 max-h-24 overflow-y-auto bg-gray-50 p-3 rounded-md border">
                                    {selectedMission.detectedSites.map((site, index) => (
                                        <div key={index} className="text-sm">
                                            <span className={`font-semibold px-2 py-0.5 rounded-full text-xs mr-2 ${site.type === 'Open' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>{site.type}</span> 
                                            {site.object}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="grid grid-cols-4 gap-4 pt-4 mt-4 border-t items-center">
                            <div><p className="text-xs text-gray-500">Status</p><p className="font-semibold">{selectedMission.status}</p></div>
                            <div><p className="text-xs text-gray-500">Duration</p><p className="font-semibold">{selectedMission.duration}</p></div>
                            <div><p className="text-xs text-gray-500">Location</p><p className="font-semibold">{selectedMission.location}</p></div>
                             <div>
                                <button 
                    ...
                    <div className="flex items-center justify-center h-full">
                        <p className="text-gray-500">Select a mission to view details or adjust your search filters.</p>
                    </div>
                 )}
            </div>
        </div>
